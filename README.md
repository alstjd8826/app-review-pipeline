...
`npm run run` 이 부르는 5단계다. 순서는 `scripts/run.sh` 가 강제한다.

```
① DB 내려받기   db-sync.ts pull
     오브젝트 스토리지 → data/pipeline.db
     ⚠️ 파싱하지 않는다. 내려받은 파일이 곧 SQLite DB 다

② 수집          ingest.ts
     각 소스에 fetchSince()          스토어에 "새 것 있냐" 질의
     play.bulkImport()               리포트 버킷에서 과거분 (선택)
     store.upsertReviews()           리뷰 ID 로 대조 — 있으면 갱신, 없으면 INSERT
     store.reconcile()               스토어에 이미 등록된 답변이 있나 대사

③ 처리          process.ts
     미처리 건만 골라서
       분류 (LLM)      → 가드레일 ① 입력 게이트
       초안 (LLM)      → 가드레일 ② 출력 검증 · ③ 신뢰도 임계

④ 알림          notify.ts
     store.unnotified()              아직 알리지 않은 건
     skip_before 필터                가동일 이전 리뷰는 전송 제외
     슬랙 전송 → 발송 기록 UPDATE

⑤ DB 올리기     db-sync.ts push
     data/pipeline.db → 오브젝트 스토리지
```

스토리지를 만지는 것은 ①과 ⑤ 뿐이다. **그 사이는 전부 로컬 SQLite 다.**
`remote.ts` 에 SQL 이 없고 `db.ts` 에 `fetch` 가 없다. 둘은 서로를 import 하지 않는다.
스토리지는 파일을 맡아둘 뿐 내용을 모른다 — 질의는 전부 SQLite 가 받는다.

⚠️ **①②③ 이 실패하면 거기서 멈춘다.** 반쪽짜리 DB 를 올려 원격을 망가뜨리지 않기 위해서다.
④(알림)는 실패해도 계속 간다. 알림 채널이 잠깐 죽어도 수집 결과는 저장돼야 한다.

### "비교"는 두 군데서 서로 다른 것을 본다

| | 무엇을 보나 | 그래서 |
|---|---|---|
| `upsertReviews()` | 리뷰 ID 가 DB 에 있나 | 같은 리뷰를 두 번 저장하지 않는다 |
| `unnotified()` | 이 건을 **알린 적 있나** | 어제 알린 건은 오늘 다시 안 쏜다 |

⚠️ **둘을 헷갈리면 로그를 잘못 읽는다.** `저장 17건` 은 새 리뷰 17건이 아니라
17건을 확인했고 중복은 갱신했다는 뜻이다. 검수 대기가 남아 있어도 이미 알렸으면 조용하다.

⚠️ **`skip_before` 이전 리뷰도 DB 에는 쌓이고 분류·초안까지 만들어진다.** 전송만 건너뛴다.
가동 첫날 과거 리뷰가 한꺼번에 쏟아지는 것을 막는 장치다.
...
⚠️ **Actions 스케줄은 정시가 아니다.** 실측 33분 지각. 정각(`:00`)과 30분(`:30`)은
전 세계 cron 이 몰려 가장 많이 밀리므로 애매한 분으로 잡는다. 도착 시각을 맞추려면
예약을 그만큼 앞당긴다. 정시가 보장돼야 하면 Cloud Scheduler 로 옮긴다. — `GUIDEBOOK` 12.7

⚠️ **공개 저장소는 60일간 활동이 없으면 GitHub 이 스케줄 워크플로를 끈다.**
커밋만 활동으로 친다 — 이슈·태그·릴리스는 해당되지 않는다. 수동 실행하면 다시 켜진다.
비공개 저장소도 해당되는지는 문서가 명시하지 않는다(공개 저장소만 언급). 커밋이 꾸준하면 겪을 일이 없다.
